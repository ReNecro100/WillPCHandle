            ...
            if (PCdata.Length < 3 || toUpdate)
            {
                ...
                using (var connection = new SqliteConnection("Data Source=data.db"))
                {
                    ...
                    foreach (ManagementObject c in cpu.Get())
                    {
                        ...
                        commandConfig2.CommandText = $"INSERT INTO Config(gpu, cpu, ram, os, directx) VALUES ({gpuId}, '{cpuId}', '{ramCounter.NextValue() + " МБ"}', '{osId}', '{directId}')";
                        ...
                        PCdata += "Свободная оперативная память: " + ramCounter.NextValue() + '\n';
